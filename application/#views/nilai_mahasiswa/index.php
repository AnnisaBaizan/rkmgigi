							...
							<?php 
								foreach($all_nilai as $nilai){
									echo "<tr>
										...
										<td>".($nilai['nilai'] <> '' ? number_format($nilai['nilai'],2) : '')."</td>
										<td>$nilai[tanggal_praktek]</td>
										<td>$nilai[thn_akademik]</td>
										<td>".($nilai['kd_semester']==1 ? 'Gasal' : ($nilai['kd_semester']==2 ? 'Genap' : ''))."</td>
										<td>"; 
										if($nilai['nilai']<>''){
										?>
										<a href="<?php echo site_url('nilai_mahasiswa/detail/'.$nilai['nim'].'/'.$nilai['id_sub_praktikum'].'/'.$nilai['id_konf']); ?>" class="btn btn-primary btn-xs"><span class="fa fa-search"></span> Detail</a>
										<a href="<?php echo site_url('nilai_mahasiswa/prints/'.$nilai['nim'].'/'.$nilai['id_sub_praktikum'].'/'.$nilai['id_konf']); ?>" target="_blank" class="btn btn-danger btn-xs"><span class="fa fa-print"></span> Print</a>
										<?php 
										}
										echo "</td>
									</tr>";
